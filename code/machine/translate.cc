...
TranslationEntry *GetEntryFromPageTable() {
  //first we get the entry from the pagetable
  int bad_addr;
  unsigned int vpn, offset;
  TranslationEntry *entry;
  int rand;
  bad_addr = machine->registers[BadVAddrReg];
  vpn = (unsigned)bad_addr/PageSize;
  offset = (unsigned)bad_addr % PageSize;
  //first we get the entry from the pageTable.
  entry = &pageTable[vpn];
  return entry;
}
...
void TlbUpdate() {
  TranslationEntry *entry;
  entry = GetEntryFromPageTable();
  //then we replace the value with the tlb.
  rand = random()%4; //we get a random value range(0-3);
  DEBUG('a',"We choose %d entry to replaced\n",rand);
  machine->tlb[rand] = entry;
}
#endif

//the second method
#ifdef TLB_FIFO
void Machine::TlbUpdate() 
{
  int lastid,id;
  TranslationEntry *entry;
  entry = GetEntryFromPageTable();
  lastid = machine->registers[TLBFIFOReg];
  id = (lastid +1)%TLBSize;
  machine->tlb[id] = entry;
  machine->registers[TLBFIFOReg] = id;
}
#endif
